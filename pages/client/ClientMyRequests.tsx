...
const ClientMyRequests: React.FC = () => {
  ...
  useEffect(() => {
    if (impersonatedClient) {
      setLoading(true);
      api.fetchClientRequests(impersonatedClient.id).then(data => {
        setRequests(data);
        setLoading(false);
      });
    } else {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-800 dark:text-white mb-6">{t('my_requests')}</h1>
      <Card>
        {loading ? <Spinner /> : (
              ...
              <tbody>
                {requests.map(request => (
                  <tr key={request.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td className="px-6 py-4 max-w-sm truncate font-medium text-gray-900 dark:text-white">{request.tenderDescription}</td>
